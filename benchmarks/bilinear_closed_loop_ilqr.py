...
class BilinearSystem:
    """
    x+[0] = a00*x[0] + u + b*u*x[0]   (bilinear)
    x+[1] = a11*x[1] + a10*x[0]        (linear)

    Linearized at (x_bar[t], u_bar[t]):
      A[t] = [[a00 + b*u_bar[t], 0], [a10, a11]]   <- depends on u_bar
      B[t] = [[1 + b*x_bar[t][0]], [0]]             <- depends on x_bar
      c[t] = [-b*u_bar[t]*x_bar[t][0], 0]           (affine offset)
    """
    a00: float
    a10: float
    a11: float
    ...
    for T in T_vals:
        for j in j_vals:
            common = dict(j=j, T=T, r_cost=r_cost,
                          ...
                          verbose=True, time_limit=cfg.time_limit)
...
def _add_kkt_chain(M, j, T, r_cost, x0_lo, x0_hi, sys):
    ...
    for k in range(j):
        ...
        # Backward costate: A^T = [[a00+b*u, a10], [0, a11]]
        ...
        for t in range(T):
            ...
            if it == 0:
                ...
                M.addConstr(
                    ...
                    name=f"lqr_dyn0_{it_tag}_{t}")
            M.addConstr(
                x_lin[t+1][1] == a10*x_lin[t][0] + a11*x_lin[t][1],
        ...
        for t in range(T - 1, -1, -1):
            if it == 0:
                ...
                M.addConstr(
                    ...
                    name=f"lqr_back0_{it_tag}_{t}")
            M.addConstr(
                lam[t][1] == Q[1, 1]*x_lin[t][1] + a11*lam[t+1][1],
